...
def recognize_captcha(test_path, save_path, image_suffix):
    ...
    print("【{}】 耗时：{}ms 预测结果：{}".format(
        now_time, int((e-s)*1000), predict_text))

    # 保存文件
    img_name = "{}_{}.{}".format(predict_text, str(
        time.time()).replace(".", ""), image_suffix)
...
def main():
    # 配置相关参数
    test_path = "sample/2019_1/b.jpeg"  # 测试识别的图片路径
